...
class glance(nn.Module):
    ...
    def forward(self, imgs, mask_ratio=0.75):
        ...
        latent = self.avgpool(latent.transpose(1, 2))
...
class CAT(nn.Module):
    ...
    def __init__(self, img_size=224, patch_emb_size=2, in_chans=3, num_classes=1000,#patch_emb_size=4,
                 embed_dim=96, depths=[1, 1 ,3 ,1 ], num_heads=[3, 6 , 12 , 6], #depths=[1, 1, 3, 1], num_heads=[3, 6, 12, 24],#96
                 patch_size=14, mlp_ratio=4., qkv_bias=True, qk_scale=None,
...
class Sum(nn.Module):
    def __init__(self): #, glance, CAT, Classification):
